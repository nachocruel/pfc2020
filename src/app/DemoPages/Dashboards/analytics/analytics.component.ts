...
export class AnalyticsComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.http.get('/appPFC/api/medicao/get_medicao_alert').subscribe((medicaoResult:any)=>{
        this.alertEmergenceList = medicaoResult.data
    }, (error)=>{
       console.log(error)
    })

    this.http.get('/appPFC/api/medicao/get_medicao_normal').subscribe((medicaoResult:any)=>{
  ...
  open(content, device) {
    let self = this
    this.http.post('/appPFC/api/device/get_last_medicao', device).subscribe((medicao:any)=>{
       if(medicao.success){
  ...
  serchDevice(content1, id_dispositivo){
    let self = this
    this.http.get(`/appPFC/api/device/${id_dispositivo}`).subscribe((device:any)=>{
